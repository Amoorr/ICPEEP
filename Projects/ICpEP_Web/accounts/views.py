...
def admin_login(request):
    """
    Login view for admins and superusers. Only admins or superusers can access this page.
    """
    if request.user.is_authenticated:  # Check if the user is already logged in
        # Redirect to respective dashboard based on the user's role
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user = authenticate(request, username=email, password=password)
            if user is not None:
                # Check for role or superuser status
                if user.is_superuser:
                    login(request, user)
                    return redirect('accounts:superuser_dashboard')  # Redirect to superuser dashboard
...
def change_password(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Pass validation errors for the new password
...
def admin_configure(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.user.is_superuser:
                ...
                return redirect('admin_configure')
        else:
            # Pass validation errors for the new password
            new_password = form.cleaned_data.get('new_password1', '')
            try:
    ...
    return render(request, 'accounts/admin/admin_configure.html', {
        'form': form,
        'password_errors': password_errors,
    })
